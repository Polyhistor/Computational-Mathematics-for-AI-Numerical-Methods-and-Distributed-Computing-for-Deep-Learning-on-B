...
%% article.
\begin{abstract}
    The convergence of computational mathematics and scalable AI systems has become critical as deep learning models grow in complexity and data volumes expand exponentially. This systematic literature review analyzes 77 peer-reviewed studies (2016-2024) following PRISMA guidelines to examine numerical methods and distributed computing techniques for deep learning on big data. Our analysis reveals three key findings: First, optimization technique selection is highly domain-dependent, with healthcare favoring nature-inspired algorithms (62.5\% adoption), cybersecurity preferring Bayesian methods (53.8\% adoption), and finance employing evolutionary algorithms (41.2\% adoption). Second, distributed computing has achieved significant scaling advances, with federated learning maintaining efficiency up to 800 client nodes and hybrid parallelism achieving near-linear scaling across 64 GPUs. Third, a concerning theory-practice gap exists where widely-adopted nature-inspired algorithms (42\% of approaches) lack theoretical rigor despite empirical success. The field is transitioning from general-purpose to specialized, context-aware optimization frameworks that balance multiple objectives including accuracy, efficiency, and privacy preservation. Future research should strengthen theoretical foundations for metaheuristic approaches, develop standardized benchmarks, and advance hardware-aware optimization to enable next-generation AI systems.
\end{abstract}
...
\keywords{Numerical Methods, Algorithms, Deep Learning, Big Data}
...
The paper is structured as follows: \Cref{sec:background} introduces key terminology and presents the rationale and objectives of the review. \Cref{sec:related-work} surveys prior literature on numerical methods and distributed frameworks for deep learning at scale and position the current study in the body of knowledge. \Cref{sec:methodology} details the systematic literature review methodology, including research questions, search strategy, inclusion and exclusion criteria, and quality assessment protocols. \Cref{sec:findings-and-analysis} presents the results and analysis of the selected studies for the SLR. \Cref{sec:discussion} interprets the findings, discusses implications, limitations, and future research directions. \Cref{sec:conclusion} concludes the paper with a summary of contributions and recommendations for advancing computational mathematics for AI.
...
The initial search execution across seven academic databases yielded a comprehensive corpus of 2,041 studies, demonstrating the extensive scope of research in computational mathematics for AI. \Cref{tab:database_coverage} presents the detailed breakdown of initial retrieval by database, revealing significant variation in yield across different academic sources.
...
% Our screening process employed a systematic reduction methodology with clearly defined criteria at each phase. \Cref{fig:study_selection_flow} illustrates the complete selection flow with quantitative metrics at each stage.
...
\begin{enumerate}
    ...
    \item \textbf{Round } employed nominal group techniques \citep{delbecq1971group} to develop operational screening guidelines addressing: (1) quantitative thresholds for big data characteristics, (2) minimum reporting standards for numerical methods, and (3) decision rules for hybrid approach classification.
...
\subsection{Phase 5: Synthesis and Reporting}\label{subsec:phase-7-synthesis-and-reporting}
...
\Cref{tab:consensus_analysis} presents the detailed analysis of inter-reviewer agreement patterns across the 132 papers entering quality screening, revealing specific areas of disagreement that required consensus resolution.
...
\begin{itemize}
    ...
    \item \textbf{Disputed paper resolution success}: 100% of disputed papers achieved final consensus
\end{itemize}
...
\section{Findings}\label{sec:findings-and-analysis}
...
This systematic review of 77 studies (2016-2024) reveals that computational mathematics for AI has evolved toward domain-specific, multi-objective optimization approaches that balance accuracy, efficiency, and scalability for deep learning on big data.

Our analysis identified three critical patterns. First, optimization technique selection is highly domain-dependent: healthcare applications favor nature-inspired algorithms (62.5\% adoption, 27.4\% accuracy gains), cybersecurity applications prefer Bayesian methods (53.8\% adoption) for uncertainty quantification, and financial applications use evolutionary algorithms (41.2\% adoption) for multi-objective optimization. Second, distributed computing has achieved significant scaling advances, with federated learning maintaining efficiency up to 800 client nodes and hybrid parallelism achieving near-linear scaling across 64 GPUs. Third, a concerning theory-practice gap exists where widely-adopted nature-inspired algorithms (42\% of approaches) lack theoretical rigor compared to gradient-based methods.

The field requires stronger theoretical foundations for popular metaheuristic approaches, standardized evaluation benchmarks, and adaptive frameworks for scaling beyond current limits. Hardware-aware optimization through algorithm-architecture co-design represents a promising direction for efficiency improvements.

This analysis reveals computational mathematics for AI is transitioning from general-purpose to specialized, context-aware optimization frameworks. Success depends on addressing the theory-practice gap while advancing scalability, efficiency, and privacy preservation for next-generation AI systems.
