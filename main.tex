...
The intersection of computational mathematics and artificial intelligence has become a critical research domain due to the growing 
complexity of deep learning models and the massive scale of data they process. As deep learning expands across applications in healthcare, 
cybersecurity, finance, and beyond, ensuring computational efficiency, scalability, and reliability has become essential. 
This has intensified interest in two foundational pillars: numerical optimization methods and distributed computing architectures.

\textbf{Numerical methods} are central to the training and tuning of deep learning models. Optimization algorithms such as gradient descent, 
Bayesian optimization, and evolutionary strategies form the backbone of neural network learning. The evolution from general-purpose optimization 
algorithms toward domain-specific techniques is well-documented in the literature. For example, nature-inspired algorithms have shown strong performance 
in healthcare applications, particularly for medical imaging and disease prediction tasks~\citet{Ampavathi20211146}, while Bayesian methods dominate in cybersecurity due 
to their capacity for uncertainty quantification~\citet{Thoppil2021,Sagu202535,Kanchanamala20232414IJACS}. Foundational work on backpropagation has been extended 
through rigorous studies analyzing convergence, complexity, and performance bounds of modern optimization techniques:contentReference[oaicite:1]{index=1}.

Despite the empirical success of many such methods, the review highlights a disconnect between theoretical rigor and practical adoption. 
Algorithms with weaker theoretical foundations, such as certain nature-inspired approaches, are widely used but often lack convergence 
guarantees:contentReference[oaicite:2]{index=2}. This gap indicates a need for more robust analysis frameworks that can bridge mathematical 
foundations and empirical deployment.

\textbf{Distributed computing}, in parallel, provides the infrastructure needed to execute these computationally intensive algorithms at scale.
 As datasets grow in volume, velocity, and variety—as articulated in the “3V” model of big data by Laney [30]—traditional single-node processing becomes 
 infeasible. Distributed computing addresses this by enabling parallelism across GPU clusters, edge devices, or federated learning environments. 
 Techniques such as federated learning allow model training without centralized data storage, improving privacy while maintaining 
 performance% :contentReference[oaicite:3]{index=3}.

In recent years, there has been an increasing trend toward hardware-aware optimization, where algorithm design explicitly considers the computational
 constraints of the deployment environment (citation). This shift acknowledges that optimal performance cannot be achieved 
 by algorithm design alone—it requires co-optimization with the underlying system architecture.

Previous reviews, such as Najafabadi et al. [36], have discussed the challenges of applying deep learning to big data, but lacked an integrated 
computational mathematics perspective. Similarly, Zhang et al. [48] offered a broad overview of distributed deep learning frameworks without deeply 
analyzing the numerical optimization strategies involved. This highlights a gap in the literature: while scalability and infrastructure 
have been discussed, the mathematical techniques enabling efficient learning have often been overlooked %:contentReference[oaicite:5]{index=5}.

This review addresses this gap by conducting a dual systematic literature review (SLR) of numerical methods and distributed computing techniques 
in deep learning for big data. The study applies rigorous methodologies based on PRISMA guidelines [35] and Kitchenham’s framework for systematic 
software engineering reviews [25], incorporating meta-analysis, network analysis, and a Delphi-based consensus protocol for inclusion and quality 
assessment:contentReference[oaicite:6]{index=6}. By synthesizing research across algorithmic and infrastructural domains, this review offers a 
holistic foundation for advancing scalable AI systems.
...
% Deep learning has significantly advanced a wide array of big data applications, delivering state-of-the-art performance across numerous domains~\citet{najafabadi2015deep,furht2016deep,choudhary2022recent}. 
% However, as neural networks become deeper and datasets more voluminous, they pose considerable challenges in terms of 
% computational scalability and efficiency~\citet{thompson2020computational,mayer2020scalable,capra2020hardware}. To meet these demands, the research community has increasingly focused on two foundational domains: numerical optimization and distributed computing. Together, they constitute the computational substrate for scalable deep learning systems.

% \textbf{Numerical methods}, especially optimization algorithms, are indispensable for training deep models. They govern the convergence behavior, learning speed, and stability of training processes. \citet{najafabadi2015deep} provide a seminal overview of deep learning in big data analytics, highlighting the computational bottlenecks and emphasizing the necessity for efficient numerical approaches to cope with high-dimensional and heterogeneous data. In response, various optimization paradigms have emerged—from gradient-based to nature-inspired and Bayesian methods—each designed to handle different objective functions and learning constraints.

% Simultaneously, \textbf{distributed computing} has grown into a core enabler of large-scale AI. It allows training over distributed resources, often using clusters, GPUs, or edge devices to reduce computation time and enhance scalability. \citet{yan2023computational} offer a comprehensive exploration of computational frameworks underpinning deep learning, discussing GPU acceleration, federated learning, and multi-node parallelism. Distributed architectures are especially pivotal for real-time applications and resource-constrained environments.

% Further synthesizing this area, \citet{zhang2023distributed} chart the progression from traditional distributed machine learning to advanced distributed deep learning systems. They evaluate the capabilities of various frameworks, such as those leveraging federated learning and GPU clustering, against scalability, fault tolerance, and deployment efficiency.

% Dedicated studies such as \citet{li2019federated} examine federated learning in greater depth, framing it as a privacy-aware, decentralized approach suitable for sensitive big data contexts. Their work underscores the architectural principles and empirical challenges in enabling collaborative model training without data centralization. Expanding on this, \citet{li2020survey} survey a broad range of scalable deep learning technologies, identifying both hardware-level (e.g., GPU, ASIC) and software-level (e.g., Spark, TensorFlow) strategies for reducing training time and enhancing performance in large-scale environments.

% In the space of optimization for deep learning, \citet{ben2019demystifying} provide a focused review of methods tailored to big data settings. They assess the effectiveness of different algorithms on convergence speed, cost-efficiency, and performance accuracy—highlighting the trade-offs faced when deploying these models at scale. Foundational insights from \citet{bengio2012practical} have guided many of these optimization strategies, while early breakthroughs like \citet{krizhevsky2012imagenet} and \citet{dean2012large} demonstrated the viability of GPU and distributed training for massive neural networks. More recent works, including \citet{shazeer2017outrageously} and \citet{you2019large}, have continued this evolution with innovations in model architecture and parallel optimization.


% While individual studies have extensively examined numerical methods or distributed systems in isolation, there is a noticeable lack of integrative reviews that systematically assess their interplay within the broader computational landscape of deep learning for big data. For instance, \citet{najafabadi2015deep} discuss the challenges of deep learning at scale, but without a detailed mathematical treatment of optimization techniques. Similarly, \citet{zhang2023distributed} and \citet{li2020survey} focus on distribution frameworks, yet omit rigorous analysis of the numerical underpinnings.

% This review aims to bridge that gap by offering a dual-focused systematic literature review that combines insights from both numerical optimization and distributed computing. It adheres to PRISMA guidelines and employs a structured methodology for literature synthesis, performance analysis, and classification. In doing so, it not only clarifies the current state of the art but also identifies critical synergies, trade-offs, and research opportunities that have been overlooked when these two pillars are studied in isolation.

% By integrating these computational perspectives, this work contributes to the development of more efficient, scalable, and robust deep learning systems—essential for addressing the complexity of modern AI applications in healthcare, cybersecurity, finance, and beyond.
...
By integrating these complementary domains through a structured review protocol grounded in PRISMA guidelines, our work reveals cross-domain synergies, unresolved trade-offs, and critical research gaps. We argue that only by examining optimization and infrastructure in tandem can the field develop robust, scalable, and efficient deep learning systems suited for complex, high-stakes applications in healthcare, cybersecurity, and beyond.
...
% \begin{figure}
%     \centering
%     \includegraphics[width=\textwidth]{media/prisma1.png}
%     \caption{Paper Classification Decision Framework}
%     \Description{Paper Classification Decision Framework}
