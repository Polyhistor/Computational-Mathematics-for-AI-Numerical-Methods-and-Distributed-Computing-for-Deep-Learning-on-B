...
\usepackage{tabularx}
\usepackage{booktabs}
%\usepackage{natbib}  % Not needed with acmart class
...
% \citestyle{acmnumeric}  % Not needed with acmart class
...
This review aims to bridge that gap by offering a dual-SLR framework that systematically evaluates both algorithmic and architectural components enabling deep learning at scale. The contribution of this work is threefold. First, it provides a structured taxonomy of numerical methods specifically tailored to big data deep learning contexts, with performance metrics such as computational efficiency and convergence behavior. Second, it evaluates distributed computing techniques (including federated learning, GPU acceleration, and parallel processing) in terms of scalability and system throughput. Third, it integrates findings across both domains to surface synergies, trade-offs, and research gaps that are often overlooked when these topics are studied in isolation.
...
%\label{subsec:conceptual-background}
...
This research adopts a SLR framework to explore two complementary yet distinct domains within the field of deep learning for big data: (SLR1) numerical methods and (SLR2) distributed computing techniques. The methodology employs a seven-phase approach combining systematic reviews with subsequent meta-analysis and network analysis.
The SLR methodology is informed by the guidelines outlined by \citet{kitchenham2004procedures}. Reviews are constructed around well-formulated research questions that guide the scope and structure of this paper.
...
The research questions were considered to address both the technical specifications and performance characteristics of deep learning systems for big data. SLR1 focuses on identifying  numerical methods and evaluating their computational efficiency and accuracy:
\begin{itemize}
...
Whereas SLR2 examines distributed computing techniques and their effectiveness in scaling deep learning applications:
\begin{itemize}
...
To ensure systematic analysis across all coauthors, we adapted Cooper's taxonomy of literature reviews \citep{cooper1988organizing} as our classification framework (\cref{fig:decision-flow2}). This approach examines studies through six critical lenses: the focus of research (ranging from theoretical to applied), the review's goal (integration vs. critical analysis), the author's perspective (neutral or advocacy), coverage breadth, organizational approach, and intended audience.
...
For SLR1, we constructed  search strings combining "deep learning" with numerical method terminology ("numerical method", "computational mathematics", "optimization algorithm") and big data.

SLR2 uses keyswords such as "distributed computing", "parallel processing", "GPU acceleration",  while maintaining the deep learning and big data context. The search strings for both SLR1 and SLR2 were adapted for each database's syntax requirements.
...
Our search spans seven major academic databases (IEEE Xplore, ACM Digital Library, SpringerLink, Scopus, Web of Science, JSTOR, AIS) to ensure broad coverage of both computer science and applied mathematics literature.
...
The query results of each of the seven databases were exported to CSV format. \ATM{The initial search yielded approximately 12,000 papers across all the seven databases.}

The CSV format was then uploaded to Baserow software [??], where duplicate papers were removed. This process eliminated nearly 30\% of initial results, highlighting the substantial overlap across the seven databases.
...
The screening was carried out by the coauthors of this paper, and the initial screening phase revealed unexpected challenges in inter-rater reliability with Krippendorff's alpha measuring only 0.4 across coauthor teams. This low agreement prompted the implementation of a modified Delphi protocol, based on RAND/UCLA methodology \citep{fitch2001rand} and Dalkey's classical Delphi framework \citep{dalkey1969delphi}.

In Round 1, each coauthor independently assessed 50 randomly selected papers, documenting detailed rationales for inclusion/exclusion decisions. The anonymous compilation of these assessments revealed three primary areas of disagreement: interpretation of "big data" characteristics, assessment of methodological rigor, and classification of hybrid numerical-distributed approaches. Round 2 provided controlled feedback, presenting coauthors with statistical summaries of group responses alongside anonymized rationales for divergent assessments and written feedback from each coauthor on points of disagreement. Round 3 employed nominal group techniques \citep{delbecq1971group} to transform these insights into operational criteria. Through structured discussion, we developed explicit screening guidelines addressing each major point of disagreement, including: Operational definitions of big data characteristics (volume, velocity requirements), minimum methodological reporting standards for numerical methods and decision rules for classifying hybrid approaches. These criteria were codified in a  decision flowchart (\cref{fig:decision-flow2}).

Following the initial screening rounds, Round 4 served as a validation phase to verify inter-rater reliability. The decision to proceed was determined by Krippendorff's $\alpha$ coefficient:  If $\alpha \geq  0.8$, indicating sufficient agreement, then the study progressed to full screening.  If $\alpha < 0.8$, the coauthors reverted to Round 3 procedures, implementing focused discussions to resolve persistent discrepancies before re-evaluating reliability.
...
Following the Delphi-based consensus methodology outlined by \citet{dalkey1969delphi} and the
systematic review guidelines of \citet{kitchenham2004procedures}, we conducted a structured consensus meeting to
establish classification criteria. The meeting employed the Nominal Group Technique as described
by \citet{delbecq1971group}, resulting in a formalized decision framework.

\paragraph{Technical Foundation Verification:} The first gate confirmed each paper's relevance to our core technical domains, requiring explicit evidence of deep learning implementation coupled with either numerical methods or distributed computing approaches.

\paragraph{Big Data Context Assessment:} We required papers to demonstrate  engagement with the four big data characteristics: substantial data volume, real-time processing requirements, heterogeneous data integration, or specialized computational infrastructure.

\paragraph{Contribution Typology Classification:} The final gate classified the articles by their primary research contribution type, application-oriented, or hybrid.
...
For hybrid contributions, we implemented a primary contribution analysis that weighted methodological innovation against domain application. Novel approaches that challenged existing classification schemes underwent third-coauthor evaluation and potential framework adaptation, with all such cases documented for transparency.

Validation occurred through three complementary mechanisms. Pilot testing on 50 sample papers confirmed the framework's  effectiveness while identifying cases for refinement. Another coauthor provided external validation of our classification logic, particularly for edge cases. Finally, statistical validation demonstrated strong agreement rates ($>85\%$ overall) and consistency of decision across the coauthors.
...
Our assessment criteria examine four  dimensions of research quality, each addressing distinct aspects of scholarly rigor and practical value. The first  establishes the minimum quality threshold, requiring studies to demonstrate systematic research methodologies rather than anecdotal evidence or untested assertions. Papers must clearly articulate their research objectives and contextual foundations, providing readers with sufficient information to evaluate the study's premises and potential limitations.

The rigor dimension evaluates the appropriateness and robustness of the research design, examining whether the chosen methodologies align with the stated objectives and whether data collection approaches yield valid, reliable evidence.

Credibility forms our third assessment dimension, focusing on the transparency and objectivity of reported findings. In a field often characterized by competing optimization claims and performance benchmarks, we pay particular attention to whether studies present results comprehensively, acknowledging limitations and potential biases. The final relevance dimension ensures the research contributes meaningfully to either academic knowledge or practical applications.

\subsubsection{Assessment Methodology}\label{subsubsec:phase-3-quality-assessment:assessment-methodology}
The quality assessment process employs a two-phase approach designed to  identify high-quality studies while maintaining rigorous standards. Phase 1 serves as an initial filter, applying only the minimum quality threshold criteria to quickly eliminate studies lacking fundamental methodological soundness.

Papers passing Phase 1 advance to comprehensive evaluation in Phase 2, where coauthors assess all remaining quality dimensions. Each criterion receives a binary 'yes/no' assessment, with studies requiring at least 75\% positive evaluations across all criteria to qualify for inclusion.

To maintain assessment consistency coauthors independently evaluate each study, with their judgments compared using Krippendorff's $alpha$ coefficient. Our target reliability threshold of $\alpha > 0.8$ reflects an acceptable standards necessary for systematic reviews in technical domains.
...
Testing on a representative sample of 50 papers allowed to refine assessment criteria and verify scoring consistency.
...
Our extraction methodology employs NVivo \cite{kitchenham2007guidelines} qualitative analysis software to  code and organize study contents according to a hierarchical framework encompassing four primary dimensions. The Method dimension captures technical specifications, including detailed descriptions of numerical algorithms or distributed computing techniques (M-01), their implementation approaches (M-02), and validation methodologies (M-03). This proves particularly crucial for enabling comparative analysis across studies with varying technical implementations.

The context dimension documents essential environmental factors that influence research outcomes, including problem domains (C-01), dataset characteristics (C-02), and computing environments (C-03). Capturing these contextual elements allows to properly situate each study's findings within its specific experimental conditions.

For the results dimension, we extract quantitative outcomes (R-01) using standardized metrics wherever possible, along with any comparative analyses (R-02) and assessments of statistical significance (R-03). This structured approach to performance data enables meaningful cross-study comparisons despite variations in reporting formats. The Findings dimension then captures higher-level insights, including key contributions (F-01), acknowledged limitations (F-02), and suggested future directions (F-03), which collectively inform our gap analysis and research agenda.
...
This phase employs two complementary analytical approaches; the meta-analysis and network analysis which together provide both quantitative and structural understanding of the research landscape.

\subsubsection{Meta-Analysis Methodology}\label{subsubsec:phase-5-combined-analysis:meta-analysis-methodology}

Our meta-analytical approach accounts for the inherent heterogeneity across studies in both technical implementations and experimental conditions. The studies may vary between studies due to differences in algorithms, architectures, datasets, and evaluation metrics. For common outcome measures such as computational efficiency and scalability improvements, visualization were done through forest plots, which  serve multiple analytical purposes.


\subsubsection{Network Analysis Framework}\label{subsubsec:phase-5-combined-analysis:network-analysis-framework}

\ATM{Network analysis examines interconnected papers through
    a comprehensive network graph for visualization, community detection identifies clusters of densely connected nodes that are similar; and centrality measure analysis quantifies node importance.}
...
\begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{media/flow-diagram.png}
...
\begin{figure}[ht]
    ...
    \Description{Distribution of research methodologies in computational mathematics for AI ($N=125$).}
...
\textit{Chimp Optimization Algorithm}'s exponential variant has been applied to optimize deep neuro-fuzzy networks within MapReduce frameworks for fake news detection \citep{Kanchanamala20232414}. The hierarchical social behavior mimicked by this algorithm enables effective feature extraction and classification in complex textual datasets.\citeauthor{Kanchanamala20232414} demonstrated a 15.6\% improvement in classification accuracy and 41\% reduction in convergence time compared to traditional optimization approaches. Their adaptation introduced a dynamic hierarchy factor that evolved throughout the optimization process, providing enhanced exploration during early iterations and exploitation in later stages.
